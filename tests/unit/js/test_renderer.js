...
// ---------------------------------------------------------------------------
// _formatUserContent — slash commands
// ---------------------------------------------------------------------------
console.log("\n_formatUserContent — slash commands");

test("formats slash command with args", () => {
    const input = "<command-message>scp</command-message>\n<command-name>/scp</command-name>\n<command-args>and create the PR</command-args>";
    assert.equal(_formatUserContent(input), "/scp and create the PR");
});

test("formats slash command without args", () => {
    const input = "<command-name>/context</command-name>\n            <command-message>context</command-message>\n            <command-args></command-args>";
    assert.equal(_formatUserContent(input), "/context");
});

test("formats slash command with varied tag order", () => {
    const input = "<command-message>engage</command-message>\n<command-name>/engage</command-name>";
    assert.equal(_formatUserContent(input), "/engage");
});

// ---------------------------------------------------------------------------
// _formatUserContent — system reminders
// ---------------------------------------------------------------------------
console.log("\n_formatUserContent — system reminders");

test("strips system-reminder tags", () => {
    const input = "<system-reminder>This is injected context\nwith multiple lines</system-reminder>";
    assert.equal(_formatUserContent(input), "");
});

test("strips system-reminder and preserves surrounding text", () => {
    const input = "hello <system-reminder>hidden</system-reminder> world";
    assert.equal(_formatUserContent(input), "hello world");
});

// ---------------------------------------------------------------------------
// _formatUserContent — local commands
// ---------------------------------------------------------------------------
console.log("\n_formatUserContent — local commands");
...
test("cleans local-command-stdout ANSI codes", () => {
    const input = "<local-command-stdout>\x1b[1mBold text\x1b[22m normal</local-command-stdout>";
    assert.equal(_formatUserContent(input), "Bold text normal");
});

test("handles caveat + stdout together", () => {
    const input = "<local-command-caveat>Caveat text</local-command-caveat>\n<local-command-stdout>output here</local-command-stdout>";
    assert.equal(_formatUserContent(input), "output here");
});

// ---------------------------------------------------------------------------
// _formatUserContent — passthrough and edge cases
// ---------------------------------------------------------------------------
console.log("\n_formatUserContent — passthrough and edge cases");

test("returns plain text unchanged", () => {
    assert.equal(_formatUserContent("just a normal message"), "just a normal message");
});

test("handles empty result from pure system-reminder", () => {
    const input = "<system-reminder>all hidden</system-reminder>";
    assert.equal(_formatUserContent(input), "");
});

test("handles null input", () => {
    assert.equal(_formatUserContent(null), "");
});

test("handles empty string", () => {
    assert.equal(_formatUserContent(""), "");
});

// ---------------------------------------------------------------------------
// renderBeat — empty formatted user message
// ---------------------------------------------------------------------------
console.log("\nrenderBeat — empty formatted user message");

test("renderBeat skips empty formatted user message", () => {
    const container = makeContainer();
    const result = renderBeat(makeBeat(0, {
        type: "user_message",
        category: "direct",
        content: "<system-reminder>hidden content only</system-reminder>",
    }), container);
